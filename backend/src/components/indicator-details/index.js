...
const IndicatorDetails = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const res = await IndicatorDetailsService.getIndicatorDetails().then(
      (response) => {
  ...
  const saveData = async (data) => {
    setLoading(true);
    await IndicatorDetailsService.saveIndicatorDetails(data).then(
      (response) => {
        //let value = await response.json();
        // alert(JSON.stringify(response.data));
        //setContents(response.data)
        ...
        swal(
          "Good job!",
          "Successfully Added New Indicator Details",
          "success"
        );
        addFormData.title = addFormData.description = addFormData.sampleExcelPath =
          "";
        setLoading(false);
      },
      (error) => {
  ...
  const updateData = async (data) => {
    setLoading(true);
    await IndicatorDetailsService.updateIndicatorDetails(data).then(
      (response) => {
        //let value = await response.json();
        // alert(JSON.stringify(response.data));
        //setContents(response.data)
  ...
  const handleDeleteClick = async (contentId) => {
    ...
    const config = {
      method: "delete",
      url: IndicatorDetailsService.API_URL + "indicator-details/" + contentId,
      headers: {
    ...
    const response = axios(config)
      .then(
        //const response = await IndicatorDetailsService.softDeleteSingleUser(contentId).then(
        (response) => {
  ...
  const fileData = () => {
    if (addFile) {
      return (
        <div>
          <h2>File Details:</h2>
          <p>File Name: {addFile.name}</p>

          <p>File Type: {addFile.type}</p>

          <p>Last Modified: {addFile.lastModifiedDate.toDateString()}</p>
        </div>
      );
    } else {
      return (
  ...
  const handleAddFormSubmit = (event) => {
    ...
    if (!error) {
      ...
      formData.append("measure", addFormData.measure);
      formData.append("dimensions", addFormData.dimensions);
      formData.append("feasibleCharts", addFormData.feasibleCharts);
      formData.append("indicator_id", addFormData.indicator_id);
      if (addFile) {
        formData.append("sampleExcelPath", addFile, addFile.name);
      }
  ...
  const handleEditClick = (event, content) => {
    ...
    const formValues = {
      ...
      feasibleCharts: content.feasibleCharts,
      indicator_id: content.indicator_id,
      sampleExcelPath: content.sampleExcelPath,
    };
  ...
  const handleEditFormSubmit = (event) => {
    ...
    const editedContent = {
      ...
      feasibleCharts: editFormData.feasibleCharts,
      indicator_id: editFormData.indicator_id,
      sampleExcelPath: editFormData.sampleExcelPath,
    };
    const formData = new FormData();
    formData.append("id", editContentId);
    formData.append("title", editFormData.title);
    formData.append("description", editFormData.description);
    formData.append("measure", editFormData.measure);
    formData.append("dimensions", editFormData.dimensions);
    formData.append("feasibleCharts", editFormData.feasibleCharts);
    formData.append("indicator_id", editFormData.indicator_id);
    if (editFile) {
      formData.append("sampleExcelPath", editFile, editFile.name);
    }
