...
class WorkingPoverty extends Component {
  ...
  handleForce = (data, fileInfo) => {
   ...
   /*  fetch("http://localhost:3001/api/v1/bulkcreate", {
    ...
    axios.post(API_URL+"bulkworkingpoverty", data, { headers: authHeader() })
      .then((response) => {
  ...
  loadClickHandler = () => {
    //
    this.setState({loading:true});
    //fetch(this.API_URL+"Employment")
    axios.get(API_URL+"workingpoverty", { headers: authHeader() })
      .then((response) => {
        //alert(JSON.stringify(response))
        if (response) {
          this.setState({loading:false});
...
deleteClickHandler = ()=>{
  ...
  fetch(API_URL+"bulkworkingpoverty", {
    method: "DELETE"
  })
    .then((response) => {
      this.setState({ loading: false });
