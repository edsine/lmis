...
router.post('/', requireToken, multer({ storage: storage}).single("imagePath"), function (req, res) {
...
router.get('/',  requireToken, function (req, res) {
   // helper.checkPermission(req.user.role_id, 'user_get_all').then((rolePerm) => {
      Indicator
        .findAll({
          attributes: ['id','title','content','imagePath','created_at'],
        })
...
// Update an Indicator
router.put('/:id', requireToken, multer({ storage: storage}).single("imagePath"), function (req, res) {
