...
exports.bulkCreate = (req, res) => {
  ...
  const { indicatorDetailsId } = req.params;
  ...
  const mergedBody = req.body.map((v) => ({
    ...v,
    indicator_details_id: indicatorDetailsId,
  }));


  IndicatorDetailsMeta.bulkCreate(mergedBody)
...
exports.findAll = (req, res) => {
  const { indicatorDetailsId } = req.params;
  IndicatorDetailsMeta.findAll({
    where: { indicator_details_id: indicatorDetailsId },
  })
    .then((data) => res.send(data))
    .catch((error) => {
      console.log(error);
